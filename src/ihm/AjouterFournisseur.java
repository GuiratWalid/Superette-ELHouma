...
public class AjouterFournisseur extends JFrame implements ActionListener{
	...
	public AjouterFournisseur() {
		...
		// Titre
		labelName = new JLabel("Ajouter un fournisseur");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btAjouter) {
			...
			if (!Controle.isLetters(nom) || nom.isEmpty()) {
				...
				Fournisseur c = new Fournisseur(nom,adresse,tel);
				FournisseurDAO cd = new FournisseurDAO();
				Fournisseur s = cd.ajouter(c);
				if (s != null) {
					
					JOptionPane.showMessageDialog(null, "Nouveau Fournisseur est ajouté avec succès");				
				}
				else
					JOptionPane.showMessageDialog(null, "Problème d’ajout du Fournisseur");
