...
public class ModifierEmploye extends JFrame implements ActionListener,ItemListener,WindowListener{
	...
	public ModifierEmploye() {
		...
		// Titre
		labelName = new JLabel("Modifier un employé");
		...
		idEmploye.addItemListener(this);
		pAPr.add(idEmploye);
		...
		textNom = new JTextField("Taper le nom de l'employé");
		...
		textPrenom = new JTextField("Taper le prénom de l'employé");
		...
		textTel = new JTextField("Taper le numéro de téléphone");
		...
		textAdresse = new JTextField("Taper l'adresse de l'employé");
		...
		mois_list.addItemListener(this);
		dn.add(mois_list);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btModifier) {
			...
			if (!Controle.isLetters(nom) || nom.isEmpty()) {
				...
				Employe c = new Employe(id,nom,prenom,tel,adresse,salaire,dateEmbauche,dateNaissance);
				EmployeDAO cd = new EmployeDAO();
				Employe s = cd.modifier(c);
				if (s != null) {
					JOptionPane.showMessageDialog(null, "Employé est modifié avec succès");
				}
				else
					JOptionPane.showMessageDialog(null, "Problème de modification du Employé");
				cd.fermerConnexion();
			}
		}
