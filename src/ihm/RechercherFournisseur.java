...
public class RechercherFournisseur extends JFrame implements ActionListener,WindowListener{
	...
	public RechercherFournisseur() {
		...
		this.setTitle("Rechercher un fournisseur");
		...
		// Titre
		labelName = new JLabel("Rechercher un fournisseur");
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btRechercher) {
			...
			ResultSet rs = fd.rechercherId(id);
			try {
				if(rs.next()) {
				...
				JOptionPane.showMessageDialog(null, "Problème de recherche du Fournisseur");
				e1.printStackTrace();
			}
			fd.fermerConnexion();
	...
	public void windowOpened(WindowEvent e) {
		...
		try {
			while(listeCat.next()) {
				idFournisseur.addItem(listeCat.getInt(1));
			}
			cd.fermerConnexion();
		} catch (SQLException e1) {
			// TODO Auto-generated catch block
