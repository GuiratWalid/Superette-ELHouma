...
public class ModifierProduit extends JFrame implements ActionListener,WindowListener,ItemListener {
	...
	public ModifierProduit() {
		...
		// Titre
		labelName = new JLabel("Modifier un produit");
		...
		textQuantite = new JTextField(15);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btModifier) {
			...
			if (designation.isEmpty()) {
			...
			else if (idCategorie.getSelectedItem() == null) {
				...
				idCat = Integer.parseInt(idCategorie.getSelectedItem().toString().substring(0,idCategorie.getSelectedItem().toString().indexOf("-")));
				designationCategorie = idCategorie.getSelectedItem().toString().substring(idCategorie.getSelectedItem().toString().indexOf("-")+1);
				...
				Produit s = pd.modifier(p);
				if (s != null) {
					JOptionPane.showMessageDialog(null, "Produit est modifié avec succès");	
				}
				else
